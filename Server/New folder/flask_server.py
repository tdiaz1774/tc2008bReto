    ...
    if request.method == 'GET':
        ...
        for agent in carModel.schedule.agents:
            ...
            if isinstance(agent, Car):
                ...
                # for a in carModel.grid.get_cell_list_contents(agent.pos):
                #     if isinstance(a, list):
                #         a = a[0]
                #         print("Chromala")
                #     if isinstance(a, Traffic_Light) or isinstance(a, Road):
                #         directions.append(directionStrToAngle[a.direction])
...
def getTrafficLights():
    ...
    if request.method == 'GET':
        lightsPositions = [{"x": x, "y":0, "z":z} for (a, x, z) in carModel.grid.coord_iter() if isinstance(a[0], Traffic_Light)]

        return jsonify({'positions':lightsPositions})
