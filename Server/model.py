            ...
            for (a,x,y) in self.grid.coord_iter():
                ...
                if isinstance(a, Road) or isinstance(a, Traffic_Light):
                    ...
                    if (a.direction == "Right" or a.direction == "Left") and x+move > 0 and x+move < self.width and isinstance(self.grid[x+move][y], list):
                        ...
                        for i in [1,-1]:
                            if y+(2*i) >= 0 and y+(2*i) < self.height and isinstance(self.grid[x][y+(2*i)], list) and isinstance(self.grid[x][y+(2*i)][0], Road) and isinstance(self.grid[x][y+i][0], Road) and self.grid[x][y+i][0].direction != a.direction:
                                G[x+self.width*y][x+self.width*(y+i)] = 1

                            if y+i >= 0 and y+i < self.height and isinstance(self.grid[x+move][y+i], list):
                                G[x+self.width*y][x+self.width*(y+i)+move] = INF/2

                    elif (a.direction == "Up" or a.direction == "Down") and y+move > 0 and y+move < self.height and isinstance(self.grid[x][y+move], list):
                        G[x+self.width*y][x+self.width*(y+move)] = 1

                        for i in [1,-1]:
                            if x+(2*i) >= 0 and x+(2*i) < self.width and isinstance(self.grid[x+(2*i)][y], list) and isinstance(self.grid[x+(2*i)][y][0], Road) and isinstance(self.grid[x+i][y][0], Road) and self.grid[x+i][y][0].direction != a.direction:
                                G[x+self.width*y][x+self.width*(y)+i] = 1

                            if x+i >= 0 and x+i < self.width and isinstance(self.grid[x+i][y+move], list):
                                G[x+self.width*y][x+self.width*(y+move)+i] = INF/2
            ...
            for agents, x, y in self.grid.coord_iter():
                for agent in agents:
                    if isinstance(agent, Traffic_Light):
                        agent.state = 2 if agent.state == 0 else 0
        elif self.schedule.steps % 10 == 8:
            for agents, x, y in self.grid.coord_iter():
                for agent in agents:
                    if isinstance(agent, Traffic_Light) and agent.state == 2:
                        agent.state = 1
